...
> Verified against: master branch, 2026-03-12 (pass 2 — metrics-verified, gap-filled)
...
**Key metrics:** 76 TypeScript source files (6,426 lines verified), 60 `WebviewExtension` implementations + 4 support files, 7 webview runtime JS modules, 9 test suites (~1,259 test lines, ~20% structural coverage), 28 demo shaders, 44 template injection anchors (22 HTML + 22 JS).
...
|---|---|---|---|
| **Test** | `.github/workflows/tests.yml` | push / PR to `master` | Checkout → Node 14.x → `npm install` → `xvfb-run npm test` (Ubuntu only) |
| **Publish** | `.github/workflows/publish.yml` | workflow_run (after "Tests" succeeds on `master`) | Checkout → Node **10.x** → `npm install` → `vsce publish` |

**CI risks:** Both workflows pin `actions/checkout@v1`, `actions/setup-node@v1` (current: v4, deprecated Node runners). Publish uses Node 10.x (EOL since 2021). Fragile cross-workflow trigger on check name `"Tests"` + branch `test_publish`. No matrix testing (Ubuntu only). ESLint configured but never run in CI.
...
> **`deactivate()` (lines 102–103) is empty:** No cleanup of event listeners, timers, or panels. Module-scoped `events[]` disposable array and `timeout` handle are not disposed, relying entirely on VS Code's process teardown. Latent resource-leak risk if the extension is hot-reloaded without a full window reload.

**Context facade** (`src/context.ts`, 209 lines):
...
**ShaderToyManager** (`src/shadertoymanager.ts`, 403 lines) — Central hub:
...
**Parsing layers:**
- **`ShaderStream`** (`src/shaderstream.ts`, 121 lines): Character-level stream with `mutate(begin, end, source)` for in-place text replacement. Tracks mutation ranges for `originalLine()` remapping.
- **`ShaderLexer`** (`src/shaderlexer.ts`, 351 lines): Tokenizer for directives. Recognizes `::` as single token (for `#iChannel0::MagFilter`). Tracks `LineRange` (begin/end positions) for directive removal.
- **`ShaderParser`** (`src/shaderparser.ts`, 669 lines): Recursive descent parser. Dispatches to type-specific parsers: `getInclude()`, `getVertex()`, `getTextureObject()`, `getUniformObject()`. Uniform parsing is the most complex — handles typed constructors, range arrays, step values, type assignability. Contains the only `TODO` in the entire codebase: _"Check if type is array type"_ (uniform array type validation).
...
**Template-driven architecture:** The webview HTML starts from `resources/webview_base.html` (773 lines, 44 injection anchors — 22 HTML-style `<!-- … -->` + 22 JS-style `// …`). Extensions inject/replace content at specific comment/placeholder lines.

**`WebviewContentAssembler`** (`src/webviewcontentassembler.ts`, 110 lines):
...
**`WebviewContentProvider`** (`src/webviewcontentprovider.ts`, 529 lines) — The wiring harness. Assembly order:
...
### 6.6 Hot Reload (Incremental Update Path)

On document change (after configurable delay) or save:
1. Re-parse entire shader tree
2. If webview resource roots unchanged → post `hotReload` message (avoids full panel teardown)
3. **Payload:** `{ shaderHtml, includeHtml, initScript, startingData }`
...
4. **Browser-side handling:** Replace shader/include `<textarea>`/`<script>` nodes with new HTML, re-run init script, recompile shaders, preserve all interactive state from `startingData`
5. If resource roots changed → **full webview teardown and recreation** (VSCode requires resource roots at panel creation time)

**Why this matters:** Hot reload is the mechanism that makes live-editing feel instantaneous — it avoids the flicker and state reset of a full panel rebuild.
...
**60 files implement `WebviewExtension`** + 4 support files = 64 total in `src/extensions/`:

| Non-Impl Support File | Lines | Role |
|---|---|---|
...
| `error_display_wrapper.ts` | 138 | Shared error display utility — generates `console.error` monkey-patch, WebGL error log parser, `#line`-based source mapping, and clickable error links. Common base for all three error display extensions. |

**Implementation breakdown (60 files):** 15 in `user_interface/` + 5 in `error_display/` + 40 in root `extensions/` directory.
...
| `hotReload` | `HotReloadPayload` | Live shader update (see §6.6) |
| `readDDSFileResult` | `{ requestId, ok, base64?, error? }` | Reply to `readDDSFile` request |
...
8. **Empty `deactivate()`:** No cleanup of event listeners, panels, or timers on deactivation (see §6.1). Leaked handlers may cause ghost callbacks if the extension is hot-reloaded without a window reload.
9. **CI fragility:** GitHub Actions workflows use severely outdated action versions (v1/v2); publish workflow targets Node 10.x (EOL since 2021). No matrix testing (Ubuntu only), no lint step in CI.
10. **Single TODO in codebase:** `src/shaderparser.ts` — _"Check if type is array type"_ — uniform array types may not be fully validated.
...
- **Frames Panel** (`src/framespanel.ts`, 121 lines): A dedicated `WebviewPanel`-based frame timing display registered as a separate panel alongside preview panels. Receives frame timing data from the webview via IPC — separate from the Stats.js in-webview overlay.
...
**Approximate coverage:** ~1,259 test lines / 6,426 source lines ≈ **20% structural line ratio** (not instrumented).

**Well-tested:** Lexer/parser directive recognition, error line offsets (regression suite), `void main()` injection heuristics, DDS float texture parsing, WebGL2/GLSL ES shims, webview module loading modes, texture init code generation.

**NOT tested (zero coverage):** Audio input pipeline, keyboard input, cubemap loading, multi-pass buffer wiring/ping-pong, screenshot/recording, glslify integration, first-person controls, hot reload path, portable HTML generation, config change handling, `ShaderToyManager` panel lifecycle/IPC routing.

**Structural gap:** No integration tests exercise the full `activate → open preview → edit shader → observe update` cycle. All tests are unit-level.

## 17. Source Architecture Summary

```
src/
├── extension.ts                    # VSCode activation, command registration (103 lines)
├── shadertoymanager.ts             # Webview lifecycle, IPC hub, state management, hot reload (403 lines)
├── webviewcontentprovider.ts       # Assembly orchestrator (529 lines)
├── webviewcontentassembler.ts      # Template injection engine (110 lines)
├── webviewcontent.ts               # Template file reader/mutator (39 lines)
├── bufferprovider.ts               # Shader tree parsing, directive processing (843 lines)
├── shaderparser.ts                 # Recursive descent directive parser (669 lines)
├── shaderlexer.ts                  # Tokenizer (351 lines)
├── shaderstream.ts                 # Character stream with mutation support (121 lines)
├── typenames.ts                    # Core type definitions and enums (122 lines)
├── constants.ts                    # SELF_SOURCE_ID, WEBGL2_EXTRA_SHADER_LINES (10 lines)
├── context.ts                      # VSCode API facade (209 lines)
├── framespanel.ts                  # Dedicated frame timing panel (121 lines)
├── utility.ts                      # removeDuplicates helper (13 lines)
└── extensions/                     # 60 WebviewExtension implementations + 4 support files
    ...
    └── user_interface/             # UI button/style extensions (15 files)
        └── error_display/          # GLSL error handling extensions (5 files)

resources/
├── webview_base.html               # HTML template with 44 injection anchors (773 lines)
